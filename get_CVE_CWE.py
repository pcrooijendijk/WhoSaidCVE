...
cve_pattern = re.compile(r'CVE-\d{4}-\d{4,7}', re.IGNORECASE) # Got the CVE pattern from the stackoverflow post from the link
cwe_pattern = re.compile(r'CWE-\d{2,4}', re.IGNORECASE)
go_pattern = re.compile(r'GO-\d{4}-\d{2,4}', re.IGNORECASE)
github_pattern = re.compile(r'GHSA-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}', re.IGNORECASE) # GHSA-cmp6-m4wj-q63q
...
# Getting the label(s) from GitHub API
def get_github_label(html_link):
    ...
    parts = html_link.rstrip("/").split("/")
    owner, repo, pull_id = parts[3], parts[4], parts[6]

    # https://stackoverflow.com/questions/37345224/github-get-labels-of-pull-request-from-api
    url = f"https://api.github.com/repos/{owner}/{repo}/issues/{int(pull_id)}"

    headers = make_headers(KEYS[KEY_INDEX])

    try:
        response = requests.get(url, headers=headers)
        # This error is given when the either the repo is set to private or if the current key is out of requests
        if response.status_code == 403: 
            print("Hit rate limit. Switching key...")
            g = change_key(g)
